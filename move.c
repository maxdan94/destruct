...
Info:
This program shuffle the edges of a graph by iteratively removing a randomly chosen edge and putting it back between two randomly chosen nonadjacent nodes. The input graph should not contain any self-loop or double-edge and the resulting graphs will not contain any self-loop or double-edge.
...
- The resulting graphs will be printed in the files "net1.txt", "net2.txt", ..., "netng.txt". "neti.txt" is thus obtaind by performing i*no move operations on the input graph.
...
//remove a random edge and add a
void move(edgelisthash* g){
	unsigned long k = rand() % g->e;
	edge ed;
	unsigned long tmp;
	rmedge(g,k);//remove a randomly chosen edge
