...
- The resulting graphs will be printed in the files "net1.txt", "net2.txt", ..., "netng.txt". "neti.txt" is thus obtaind by performing i*no add operations on the input graph.
...
typedef struct {
	...
	unsigned long emax;//number of edges after adding all no*ng edges
...
void mkhash(edgelisthash* g){
	...
	g->ht=malloc(g->emax*sizeof(unsigned long));//size of hashtable is g->emax
	g->ll=malloc(g->emax*sizeof(unsigned long));//using a single malloc for the linked lists
	for (i=0;i<g->emax;i++){
		g->ht[i]=-1;
		g->ll[i]=-1;
	}
	for (i=0;i<g->e;i++){//filling the hash table
		h=hash(g->edges[i],g->emax);
		if (g->ht[h]==-1){
			g->ht[h]=i;
		}
		else {
...
char isin(edgelisthash *g, edge ed){
	unsigned long h=hash(ed,g->emax);
...
//add edge ed at the last position and in the hash table
void addedge(edgelisthash *g, edge ed){
	g->edges[g->e]=ed;
	unsigned long h=hash(ed,g->emax);
	unsigned long i;
	if (g->ht[h]==-1){
...
//add a random edge
void add(edgelisthash* g){
	edge ed;
	unsigned long tmp;
	//find a non-existing edge (u,v) to add
	do{
		ed.s=rand()%g->n;
		ed.t=rand()%g->n;
		if (ed.s>ed.t){
...
int main(int argc,char** argv){
	...
	g->emax=g->e+no*ng;
	printf("Number of edges of the final graph: %lu\n",g->emax);
